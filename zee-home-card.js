...
/* ════════════════════════════════════════════════════════════════════
   ...
   customElements collision with the original casa-luna card)
...
const SL = {
  ...
  r_pvtile:[1113,420,369,50], r_ev:[1113,462,369,50],   r_cons:[1113,480,369,123], r_prod:[1113,612,369,123], r_events:[1113,745,369,154],
...
class CasaLuna extends HTMLElement {
  ...
  static getStubConfig() {
    return {
      ...
      sys_core1_temp: '', sys_core2_temp: '', sys_package_temp: '',
      sys_eth0_rx: '', sys_eth0_tx: '', sys_wlan0_rx: '', sys_wlan0_tx: '',
  ...
  _build() {
    ...
    <div style="position:absolute;left:0;top:0;width:${VB_W}px;height:140px;pointer-events:none">
      ...
      <div style="position:absolute;left:0;top:0;width:${VB_W}px;text-align:center;pointer-events:none">
        ...
        <div id="hSubtitle" style="margin-top:5px;line-height:1;font-family:'Bahnschrift','Arial Narrow','Segoe UI',sans-serif;font-size:11.2px;font-weight:400;
          letter-spacing:4.8px;color:#a8cae6;text-align:center">ENERGY • AUTOMATION • SECURITY • by Zeeshan Khan</div>
      </div>
    </div>`;
    ...
    <div class="box" style="left:${mx}px;top:${my}px;width:${mw}px;height:${mh}px;overflow:hidden">
      ...
      <div style="position:absolute;left:16px;right:14px;top:74px;bottom:8px;overflow-y:auto;display:flex;align-items:flex-start;justify-content:space-between;gap:4px">
        <span id="invStateLbl" style="font-size:11px;color:#7fa3c4;text-transform:uppercase;letter-spacing:.05em;white-space:nowrap;flex-shrink:0">${esc(c.label_inverter_state || 'INV STATE')}</span>
  ...
  _bindSlider(track, fromX, commit) {
    ...
    const end = (e) => {
      ...
      this._panelBusy = false;
      try { track.releasePointerCapture(e.pointerId); } catch (e2) { console.debug('zee-home-card: releasePointerCapture no-op', e2); }
      if (v != null) commit(v);
    };
    track.addEventListener('pointerdown', e => { this._panelBusy = true; v = fromX(e.clientX); try { track.setPointerCapture(e.pointerId); } catch (e2) { console.debug('zee-home-card: setPointerCapture no-op', e2); } });
  ...
  /* ── SYSTEM view: inverter metrics, server stats, temps, network ── */
  _viewSystem() {
    const c = this.config;
  ...
  async _renderCalendar(ov) {
    ...
    } catch (e2) { console.warn('zee-home-card: calendar fetch failed', e2); events = []; }
  ...
  _update(tick = false) {
    ...
    this._setTxt('#invStatus', Number.isFinite(invT) ? `${this._decEnt(c.inv_temp)} °C` : '');
    const radT = c.inv_rad_temp ? this._num(c.inv_rad_temp, NaN) : NaN;
    this._setTxt('#invRadTemp', Number.isFinite(radT) ? `${this._decEnt(c.inv_rad_temp)} °C` : '');
...
class CasaLunaEditor extends HTMLElement {
  ...
  _render() {
    ...
    shell.appendChild(section('nav_system', '🖥️', 'System View', [
      ...
      picker('sys_eth0_rx', 'eth0 RX', true), picker('sys_eth0_tx', 'eth0 TX', true),
      picker('sys_wlan0_rx', 'wlan0 RX', true), picker('sys_wlan0_tx', 'wlan0 TX', true),
    ]));
...
if (!customElements.get('zee-home-card')) customElements.define('zee-home-card', CasaLuna);
if (!customElements.get('zee-home-card-editor')) customElements.define('zee-home-card-editor', CasaLunaEditor);
window.customCards = window.customCards || [];
window.customCards.push({
  type: 'zee-home-card',
  name: 'Zee Home Card',
  description: 'Zee Home Card — energy dashboard — scenic background, live energy flows, clickable navigation. Based on Casa Luna by The Khan.',
  preview: false,
});
console.info(`%c ZEE-HOME-CARD %c v${VERSION} `, 'background:#0a2a55;color:#7fd4ff;font-weight:700', 'background:#123;color:#9ae63c');
})();
